...
<body>
    ...
    <script> 
        fetch('getSalesData') 								<!-- 'getSalesData' 엔드포인트(=url)로부터 데이터를 가져옴 -->
            ...
            .then(data => { 								<!-- 데이터 받아와서 처리 -->
                const labels = data.map(item => item.month); <!-- data 배열의 각 요소에 대해 month 속성을 추출하여 새로운 배열 생성 -->
                const sales = data.map(item => item.sales);  <!-- data 배열의 각 요소에 대해 sales 속성을 추출하여 새로운 배열 생성 -->

                const ctx = document.getElementById('myChart').getContext('2d'); 
                new Chart(ctx, { 												<!-- Chart.js를 사용하여 차트 생성 -->
                    type: 'bar', 												
                    data: { 													<!-- 차트에 표시할 데이터 설정 -->
